...
use garfield::{
    analyze, build_graph, detect, extract_file, find_shortest_path, score_nodes, validate_graph,
};
use std::path::Path;
...
fn test_e2e_full_pipeline_real_directory() {
    ...
    println!(
        "✓ Analysis complete: {} communities",
        analysis.community_sizes.len()
    );
...
fn test_e2e_large_graph_performance() {
    ...
    println!(
        "✓ Large graph: {} nodes processed successfully",
        graph.nodes.len()
    );
}
