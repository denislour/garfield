...
use garfield::{
    analyze, build_graph, detect_hyperedges, extract_file, find_shortest_path, 
    score_nodes, serve::{get_node, subgraph_to_text},
    GraphData,
};
...
fn edge(src: &str, tgt: &str, relation: &str) -> Edge {
    Edge::new(src.into(), tgt.into(), relation.into(), Confidence::Extracted)
}
...
fn setup_test_project() -> TempDir {
    ...
    std::fs::write(src.join("pricing.rs"), r#"
    ...
    std::fs::write(src.join("order.rs"), r#"
...
fn test_build_creates_hyperedges() {
    ...
    assert!(!graph.links.is_empty() || graph.nodes.len() > 0, "Should have content");
}
...
fn test_query_returns_nodes_with_module_tags() {
    ...
    if !scores.is_empty() {
        let top_ids: HashSet<String> = scores.iter()
            .take(3)
...
fn test_hyperedge_explains_group() {
    ...
    if let Some(he) = graph.hyperedges.first() {
        ...
        assert!(he.nodes.len() >= 3, "Hyperedge should have at least 3 members");
    }
}
...
fn test_score_nodes_ranks_by_relevance() {
    ...
    let create_scores: Vec<_> = scores.iter()
...
fn test_empty_graph_handling() {
    ...
    assert!(graph.nodes.is_empty(), "Empty extraction should produce empty graph");
    assert!(graph.hyperedges.is_empty(), "Empty graph should have no hyperedges");
    
    let output = subgraph_to_text(&graph, &HashSet::new(), &[], 1000);
    assert!(output.contains("## Nodes"), "Empty query should show ## Nodes");
}
...
fn test_hyperedge_contains_valid_nodes() {
    ...
    for he in &graph.hyperedges {
        for node_id in &he.nodes {
            ...
            assert!(exists, "Hyperedge references non-existent node: {}", node_id);
        }
    }
}
...
fn test_community_detection() {
    ...
    for i in 1..=3 {
        extraction.add_node(node(&format!("group_a_{}", i), "a.rs", &format!("group_a_{}", i)));
    }
    for i in 1..=3 {
        extraction.add_node(node(&format!("group_b_{}", i), "b.rs", &format!("group_b_{}", i)));
    }
    ...
    assert!(analysis.community_sizes.len() >= 1, "Should have at least 1 community");
}
