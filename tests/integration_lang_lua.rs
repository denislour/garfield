...
"#;
    ...
    assert!(contains_any(&nodes, &["OrderService.new", "new"]), 
        "Should find new function");
    assert!(contains_any(&nodes, &["OrderService:create_order", "create_order"]), 
        "Should find create_order, got {:?}", nodes);
    assert!(contains_any(&nodes, &["OrderService:send_confirmation", "send_confirmation"]), 
        "Should find send_confirmation");
    assert!(contains_any(&nodes, &["Order.new", "new"]), 
        "Should find Order.new");
    assert!(contains_any(&nodes, &["Order:add_item", "add_item"]), 
