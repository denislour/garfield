...
use garfield::{from_json, get_node, get_hyperedge, detect_hyperedges};
use std::path::Path;
...
fn test_hyperedge_flow_build_to_query() {
    ...
    assert!(details.hyperedge.is_some(), "Node should have hyperedge info");
...
fn test_every_node_has_hyperedge() {
    ...
    let hyperedge_nodes: std::collections::HashSet<_> = graph.hyperedges
        .iter()
