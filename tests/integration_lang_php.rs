...
"#;
    ...
    assert!(
        contains_any(&nodes, &["OrderService", "class"]),
        "Should find OrderService class, got {:?}",
        nodes
    );
    assert!(
        contains_any(&nodes, &["createOrder", "function"]),
        "Should find createOrder method, got {:?}",
        nodes
    );
    assert!(
        contains_any(&nodes, &["sendConfirmation", "function"]),
        "Should find sendConfirmation method"
    );
    assert!(
        contains_any(&nodes, &["Order", "class"]),
        "Should find Order class"
    );
    assert!(
        contains_any(&nodes, &["addItem", "function"]),
