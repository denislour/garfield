...
use garfield::{
    analyze, build_graph, cluster, detect, extract_file, find_god_nodes, find_shortest_path,
    score_nodes, validate_graph, FileCache,
};
use std::path::Path;
...
fn test_end_to_end_build() {
    ...
    // 2. Extract from a Python file (try multiple files until we find one with definitions)
    ...
    let extraction = match extraction {
        Some(e) => e,
        None => {
            println!("Could not find a file with definitions, skipping");
            return;
        }
    };
...
fn test_clustering() {
    ...
    extraction.add_edge(garfield::Edge::new(
        ...
        "a.py:B".into(),
        "calls".into(),
        garfield::Confidence::Extracted,
    ));
    extraction.add_edge(garfield::Edge::new(
        "a.py:B".into(),
        "b.py:C".into(),
        "calls".into(),
        garfield::Confidence::Inferred,
    ));
    ...
    assert!(
        result.assignments.len() > 0,
        "should have community assignments"
    );
