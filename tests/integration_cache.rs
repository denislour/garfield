...
use garfield::cache::{check_cache, compute_hash, update_cache, FileCache, CacheEntry};
...
fn test_check_cache_changed() {
    ...
    // Modify file
    let mut file = fs::OpenOptions::new().write(true).append(true).open(&file_path).unwrap();
