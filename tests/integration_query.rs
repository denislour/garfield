...
use garfield::{analyze, find_shortest_path, score_nodes, find_god_nodes, find_surprising_connections, build_graph};
...
fn test_analyze_confidence_stats() {
    ...
    extraction.add_edge(edge("A", "B"));
    extraction.add_edge(Edge::new("A".into(), "B".into(), "imports".into(), Confidence::Inferred));
    extraction.add_edge(Edge::new("A".into(), "B".into(), "uses".into(), Confidence::Ambiguous));
...
fn test_find_surprising_connections_cross_file() {
    ...
    extraction.add_node(node("A", "f1.rs"));
    extraction.add_node(node("B", "f2.rs"));
    extraction.add_edge(Edge::new("A".into(), "B".into(), "calls".into(), Confidence::Inferred));
