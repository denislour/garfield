...
## Corpus Check
- ⚠️ Small corpus: 16 files · ~25019 words
  Graph may not add much value for small codebases.

## Summary
- 510 nodes · 138 edges · 372 communities detected
- Extraction: 71% EXTRACTED · 28% INFERRED · 0% AMBIGUOUS · INFERRED: 40 edges (avg confidence: 0.85)
...
**Key concepts:** calculate_chain_cohesion, calculate_cohesion, create_test_graph, detect_call_chains, detect_config_patterns, detect_directory_groups, detect_file_groups, detect_hyperedges, find_chains_dfs, get_first_file
...
**Q:** Are the 15 inferred relationships involving `global` (e.g. with `detect` and `filter_code_files`) actually correct?

**Why:** `global` has 15 INFERRED edges - model-reasoned connections that need verification.
